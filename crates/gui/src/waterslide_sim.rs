...
fn hash01(seed: u32) -> f64 {
    Rng::new(seed).next_f64()
}

// ---- pure helpers --------------------------------------------------------
/// Map an audio offset (Hz) to a pixel row in the 0..REF_H reference space.
#[inline]
pub fn y_of(off: i32) -> f32 {
    250.0 - (off as f32) / FSPAN * REF_H
}
...
pub fn tone_of(s: i64, i: usize, step: i32) -> i32 {
    let seed = (s
        .wrapping_mul(131)
        .wrapping_add(i as i64 * 17)
        .wrapping_add(step as i64 * 7)) as u32;
...
fn message_for(s: i64, i: usize) -> (String, i32) {
    let mut r = Rng::new(
        (s.wrapping_mul(761)
            .wrapping_add(i as i64 * 53)
            .wrapping_add(11)) as u32,
    );
    let st = &POOL[i];
...
pub fn column_values(ts: f64, col_id: i64, out: &mut [f32; COL_H]) {
    ...
    for (y, slot) in out.iter_mut().enumerate() {
        let n = hash01(
            (col_id
                ...
                .wrapping_add(y as i64 * 668_265_263)
    ...
    for (i, slot) in POOL.iter().enumerate() {
        ...
        for (y, cell) in out[lo..=hi].iter_mut().enumerate() {
            let y = lo + y;
            let d1 = y as f32 - ty;
            let d2 = y as f32 - cy;
            // bright FSK core + faint full-band haze
            *cell += amp * ((-d1 * d1 / 1.62).exp() + 0.22 * (-d2 * d2 / 20.5).exp());
        }
    }
}
...
impl Sim {
    ...
    pub fn advance(&mut self, dt: f64) {
        ...
        if completed > self.max_emit {
            self.max_emit = completed;
        }
        let t = self.t;
    ...
    pub fn decodes_for_slot(&mut self, s: i64) -> Vec<Decode> {
        ...
        if overflow > 0.0 {
            for d in &mut list {
                d.final_y = (d.final_y - overflow).max(8.0);
            }
        }
