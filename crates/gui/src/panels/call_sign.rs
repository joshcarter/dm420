...
impl Panel for CallSign {
    ...
    fn ui(&mut self, ctx: &mut PanelCtx, block: Rect) {
        ...
        let grid = ctx
            ...
            // Only a Maidenhead grid is a displayable/measurable locator here; a
            // Field Day section-only spot falls through to the logbook-grid lookup.
            .and_then(|s| match s.loc {
                pd::Locator::Grid(g) => Some(g),
                pd::Locator::Section(_) => None,
            })
            .filter(|g| !g.is_empty());
