...
**Interim fix landed (2026-06).** The abort-inference half was made correct without the full loop. The carrier-abort watcher now keys on an explicit operator-Stop signal — `QsoState.abort_seq`, a counter the engine advances **only** on `QsoCommand::Abort` — instead of the `QsoPhase::Idle` edge (`core::tx::spawn_abort_watcher`; `qso::engine::on_command`). A normal completion that also reaches `Idle` (a received final sign-off → `FinishNow(Idle)`, an FD-answering `RR73` resend, a cap give-up) no longer self-aborts the legitimate final over — the regression `3a76d03` exposed once the FD-answering side stayed active to resend `RR73`. Covered by `core::tx` tests at the shell↔tx seam. **Still owed (full driver):** the closed TX-outcome loop (`Event::TxOutcome`, advance only on `Sent`), and the forward-compatible abort owned by the interlock (`Granter::revoke(token)`, step 0b) that retires *both* the `QsoState`-keyed inference (now on `abort_seq`) **and** the deferred final-state-publish hack (item 3 below). Until then `QsoState.abort_seq` is the seam to delete.
