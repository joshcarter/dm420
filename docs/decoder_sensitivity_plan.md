...
**Status:** Phase 1 (OSD), Phase 2 (coherent front-end), and Phase 3 v1
(subtraction) all done and composed. **Owner:** TBD. **Metric:** the `ab_jt9`
example (`JT9_BIN=/Applications/wsjtx.app/Contents/MacOS/jt9` on macOS).

> **The reframe (2026-06) that reordered everything.** The original plan diagnosed
> the gap as **masking (subtraction) + deep-weak (OSD)** and assumed our receive
> front-end was at WSJT-X parity. **It was not.** We are a faithful `ft8_lib` port:
> a **strictly non-coherent, magnitude-only** front-end. WSJT-X's biggest weak-signal
> wins (per-candidate fine sync + *coherent* multi-symbol integration) live *before*
> the LDPC/OSD stage. That is why OSD (Phase 1) landed with a shrug and the first
> subtraction attempt only added +9 — both consume LLRs that were ~2–3 dB degraded.
> So the coherent front-end became **Phase 2, ahead of subtraction**.
...
> - + Phase 3 subtraction v1 *on the non-coherent front-end* (`d2e0128`): **480**,
>   gap **49%** — below the masking bucket's promise (it converges by pass 3, so the
>   limiter is fit *quality* on close-spaced signals, not pass count).
> - + **Phase 2 coherent front-end** (`c078295`, alignment fixed `9ef3021`): **606**,
>   gap **35%** — the dominant win, as the reframe predicted.
>
> Phases 2 and 3 now **compose** in `decode_streaming`: each subtraction pass
> rebuilds the residual and decodes it *coherently*, and the coherent path's refined
> freq/dt feed the next subtraction's fit. `ours-only` stayed clean (≤15, all
> verified as real jt9 misses). Per-slot FT8 decode is now ~3 s (coherent is the
> cost; FT4 stays magnitude-only and single-pass) — a profiling pass is pending.
...
**Expected → measured:** ✅ this is the dominant win — matched 471 → **606**, gap
50% → **35%** (alignment fix `9ef3021`; the offset came in at one symbol / −32
downsampled, not the −16 a half-window estimate predicted). Recovered a large slice
of the weak tail and lifts every signal so Phase 3's fits start from cleaner
decodes. See `docs/decoder_phase2_handoff.md` for the detailed write-up.

### Phase 3 — multi-pass subtraction (the crowded-band lever) ✅ v1 done (`d2e0128`), now coherent

**v1 done** in `modes/src/subtract.rs`: `decode_streaming` loops decode → subtract →
rebuild waterfall → decode (FT8 only; `DM420_SUBTRACT=0` off, `DM420_SUBTRACT_PASSES`
tunes). v1 used a **per-symbol** complex-gain fit with a ±480-sample timing search,
on the *non-coherent* front-end: matched 471 → 480, no false-decode rise. After the
Phase 2 merge it now composes with the coherent path — each pass rebuilds a
`cohere::Demod` on the residual and decodes coherently, and the coherent path's
refined freq/dt feed each subtraction's fit (a free quality boost over v1's coarse
inputs). **Re-measure the standalone subtraction delta on top of coherent — the +9
was measured on the old front-end and is likely different now.**

**Phase 3.1 — the higher-quality fit (the next lever on masking).** v1's +9 was
below the masking bucket's promise because a per-symbol fit **cross-talks for
close-spaced signals** — two tones a few Hz apart aren't orthogonal over one 0.16 s
symbol, so fitting the strong signal partly absorbs the weak neighbor (exactly the
masking case). Over the full 12.6 s transmission they separate cleanly
(3 Hz × 12.6 s ≈ 38 cycles), so the fix is a **frequency-refined, more-global fit**
(what `subtractft8` does):
1. **Refine frequency to sub-Hz** before fitting — Phase 2's `cohere` fine sync
   already does this for the decode; reuse its refined `f0` for the subtraction.
2. With accurate `f0`, fit a **global complex gain** (or lightly-smoothed slow-varying
   gain to track fading) so close neighbors stay orthogonal and the residual is clean.
3. Re-measure with `ab_jt9`; subtraction converges by pass 3, so this is pure quality.

**Expected:** the −10..0 dB "should-have-decoded" misses, plus weak signals freed by
removing their maskers — multiplied now that each residual is decoded coherently.
